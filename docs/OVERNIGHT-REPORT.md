...
| 2 | **APNs environment is hardcoded to `"dev"`** (`PushCoordinator.swift:194`). | **Not a bug today, not fixed.** Both entitlement files declare `aps-environment: development`, so it is currently correct. It becomes silent push failure the moment a TestFlight or App Store build is cut. Predates this run. Recorded in the ROADMAP's latent list. |
