...
| Shared gateway room projection and profile-metadata CAS | Foundation implemented. Talaria has bounded projection normalization/merge/tombstones, a separately persisted ledger, and strict typed CAS request/response handling. AppModel hydration, all-gateway fan-out, conflict retry/read-back, reconnect reseeding, and bounded legacy fallback remain missing. | Focused Talaria projection/store and profile-CAS suites define the landed foundation; upstream group-room and profile-CAS suites remain the pinned authority | Implement the runtime integration, then run the matrix in `TESTING.md` |
