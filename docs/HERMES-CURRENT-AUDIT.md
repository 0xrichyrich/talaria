...
supported only through explicit feature detection. The implementation is
automated-test covered, but Desktop/mobile convergence remains uncertified
until the retained live matrix in `TESTING.md` passes.
...
| Shared gateway room projection and profile-metadata CAS | Implemented. Talaria combines its bounded, separately persisted ledger with safe rich-room hydration, all-credentialed-gateway fan-out, per-gateway serialization/retry, exact CAS read-back, bounded legacy fallback, and adoption/reconnect/foreground reseeding. Privacy deletion fences suspended sync, and startup applies cached tombstones before network recovery. | `RoomStoreTests`, `RoomProjectionRuntimeTests`, `ProfileUIMetaCASTests`, and the cached-tombstone `RoomRoutingTests` regression; upstream group-room and profile-CAS suites remain the pinned authority | Run and retain all six live Desktop/phone/gateway cases in `TESTING.md` |
