...
| `4e1dd1a`, `58258af`, `991af03`, `43c6dac` | Raw cron jobs gain optional `reasoning_effort`; precedence is per-job pin over per-model override over global effort. Empty clears, invalid values fail before persistence, and model tool dispatch cannot set it even though formatted jobs report it. User mutation is CLI-owned. | New portable management gap. `CronJobDetail` and `RoutineEditorView` currently cover model/provider pins but neither decodes nor presents `reasoning_effort`. A phone should at least preserve and display the raw field; whether it may mutate a CLI-owned model-configuration decision needs an explicit product decision. |
...
supported only through explicit feature detection. Until that implementation
lands, Desktop-created rooms are not promised to appear in Talaria and
Talaria-created rooms are not promised to appear in other gateway clients.
...
| Shared gateway room projection and profile-metadata CAS | Missing. Exact Hermes publishes `hermes-bots-groups` through every reachable gateway and protects concurrent per-key metadata writes with `ui_meta_revisions` / `ui_meta_expected_revisions`. Talaria currently has local room durability only and does not consume or produce the projection or CAS fields. | Absence confirmed by source search at this cutoff; upstream group-room and profile-CAS suites define the contract | Implement the decoder, merger, fan-out, tombstones, CAS retry/read-back, and legacy feature detection, then run the matrix in `TESTING.md` |
...
| Cron per-job reasoning effort | Missing. Current Hermes raw jobs may carry user-owned `reasoning_effort`, which overrides per-model and global effort for that job. Talaria neither decodes nor displays it, so an edit cannot prove preservation and the user cannot inspect the effective pin. |
