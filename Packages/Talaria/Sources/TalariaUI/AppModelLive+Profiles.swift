...
extension AppModel {
    ...
    private func applyEditLocally(botID: String, edit: ProfileEdit) {
        ...
        if let meta = edit.uiMeta?["talaria"] {
            if let raw = meta["shape"]?.stringValue, let shape = AvatarShape(rawValue: raw) {
                bots[index].shape = shape
            }
            if let raw = meta["hue"]?.stringValue, let hue = AvatarHue(rawValue: raw) {
                bots[index].hue = hue
            }
        }
    }
    ...
    public func createBotProfile(id: String, job: String, soul: String,
        ...
        let bot = Bot(id: id,
                      ...
                      shape: AvatarShape(rawValue: uiMeta["talaria"]?["shape"]?.stringValue ?? "") ?? .circle,
                      hue: AvatarHue(rawValue: uiMeta["talaria"]?["hue"]?.stringValue ?? "") ?? .teal,
                      status: .idle,
    ...
    /// profiles.get_asset → portrait cache. Cheap and idempotent: cached
    /// hits and known misses never re-hit the gateway.
    public func refreshAvatar(botID: String, force: Bool = false) async {
        ...
        guard let dataURL = try? await client.profileAvatar(name: botID),
              let data = ProfileAssetStore.decode(dataURL: dataURL) else {
...
/// model in hand (roster rows, chat header, this sheet).
        ...
        Group {
            if let data = ProfileAssetStore.shared.portrait(for: bot.id),
               let image = ProfileAssetStore.image(from: data) {
                ...
                AvatarView(bot: bot, size: size, theme: theme)
            }
        }
        .frame(width: size, height: size)
