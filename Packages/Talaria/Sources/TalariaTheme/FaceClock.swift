...
public struct FacePose: Equatable, Sendable {
    ...
    /// Amplitude and cycle of the idle sway, quoted from `facePose`'s `roll`
    /// channel (`sin(t·0.85)·1.2`) so the animated version and the sampled
    /// version cannot drift apart.
    public static let idleSwayDegrees: Double = 1.2
    public static let idleSwayPeriod: Double = 2 * .pi / 0.85
